const { User, CounselorActivity, Student, Document, Application, Note, StudentUniversityApplication, University, ApplicationCountry } = require('../models');
...
const getCounselorActivityDetails = async (req, res) => {
  try {
    ...
    // --- CALCULATE DASHBOARD STATS (Mirrors CounselorDashboard logic) ---
    // 1. Total Students
    const totalStudents = await Student.count({
      where: { counselorId }
    });

    // 2. Active Applications (students past document collection phase)
    ...
    sessionStats.completedStudents = completedEnrollments; // Update this to use the accurate calculation
