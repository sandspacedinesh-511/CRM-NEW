...
const cacheMiddleware = (ttl = 300) => {
  return async (req, res, next) => {
    ...
    const fs = require('fs');
    fs.appendFileSync('debug_log.txt', `[${new Date().toISOString()}] Generated Cache Key: ${cacheKey}\n`);
      ...
      res.send = function (data) {
        try {
          ...
          if (!res.headersSent && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            // console.log(`[DEBUG] Cache SET for key: ${cacheKey}`);
      ...
      try {
        ...
        if (req.method !== 'GET' && res.statusCode >= 200 && res.statusCode < 300) {
          ...
          for (const pattern of patterns) {
            fs.appendFileSync('debug_log.txt', `[${new Date().toISOString()}] Invalidating pattern: ${pattern}\n`);
        ...
        const fs = require('fs');
        fs.appendFileSync('debug_log.txt', `[${new Date().toISOString()}] Invalidation failed: ${error.message}\n`);
      }
