...
function DashboardLayout() {
  ...
  const [reminderDialogData, setReminderDialogData] = useState(null);
  ...
  useEffect(() => {
    ...
    const cleanup = onEvent('notification', (notification) => {
      ...
      if (notification.type === 'reminder') {
        // Extract student name and reminder details
        // WebSocket sends data directly on notification object, not in metadata
        const studentName = notification.studentName || notification.metadata?.studentName || 'Student';
  ...
  const handleProfileClick = (e) => {
    ...
    if (role === 'admin') {
      ...
      navigate('/telecaller/profile');
    } else if (role === 'marketing') {
      navigate('/marketing/profile');
    } else if (role === 'b2b_marketing') {
      navigate('/b2b-marketing/profile');
    }
  };
  ...
  const handleSettingsClick = (e) => {
    ...
    if (role === 'admin') {
      ...
      navigate('/telecaller/settings');
    } else if (role === 'marketing') {
      navigate('/marketing/settings');
    } else if (role === 'b2b_marketing') {
      navigate('/b2b-marketing/settings');
    }
  };
  ...
  const handleSecurityClick = (e) => {
    ...
    if (role === 'admin') {
      ...
      navigate('/counselor/security');
    } else if (role === 'telecaller') {
      navigate('/telecaller/security');
    } else if (role === 'marketing') {
      navigate('/marketing/security');
    } else if (role === 'b2b_marketing') {
      navigate('/b2b-marketing/security');
    }
  };
  ...
  const getBreadcrumbs = () => {
    ...
    const breadcrumbs = [
      { label: 'Home', path: homePath, icon: <HomeIcon fontSize="small" /> }
    ];
  ...
  const telecallerMenuItems = [
    ...
    { text: 'Communication', icon: <PhoneInTalkIcon />, path: '/telecaller/communication' },
    { text: 'Notifications', icon: <NotificationsIcon />, path: '/telecaller/notifications' },
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      >
        ...
        >
          <Toolbar sx={{ px: { xs: 2, md: 3 }, py: 1 }}>
            ...
            >
              ...
              <MenuItem onClick={handleSecurityClick} sx={{ py: 1.5 }}>
                <ListItemIcon>
        ...
        <Snackbar
          open={snackbarOpen}
          autoHideDuration={10000} // 10 seconds for reminders
        ...
        >
            ...
            {reminderDialogData && (
              <Box>
                <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 600 }}>
                  Student: {reminderDialogData.studentName}
                </Typography>
                {reminderDialogData.reminderTitle && (
                  <Typography variant="h6" gutterBottom color="primary">
                    {reminderDialogData.reminderTitle}
                  </Typography>
                )}
                <Typography variant="body1" sx={{ mt: 2, mb: 2 }}>
                  {reminderDialogData.reminderMessage}
                </Typography>
                {reminderDialogData.scheduledTime && (
                  <Typography variant="caption" color="text.secondary">
                    Scheduled: {new Date(reminderDialogData.scheduledTime).toLocaleString()}
                  </Typography>
                )}
              </Box>
            )}
          ...
          <DialogActions sx={{ p: 2, gap: 1 }}>
            {reminderDialogData?.studentId && (
              <Button
                variant="contained"
                color="primary"
                onClick={() => {
                  navigate(`/counselor/students/${reminderDialogData.studentId}`);
