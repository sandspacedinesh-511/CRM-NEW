...
const MultiCountryDashboard = () => {
  ...
  const getCountryStatistics = () => {
    ...
    // Helper to clean raw country strings coming from DB/notes
    ...
    // Helper to normalize for deduplication (so "USA", "US", "United States" are same)
    const normalizeCountryForDedup = (country) => {
      ...
      if (
        ['USA', 'U.S.A.', 'US', 'U.S.', 'UNITED STATES', 'UNITED STATES OF AMERICA'].includes(
          normalized
        )
      ) {
    ...
    students.forEach(student => {
      student.countries?.forEach(countryData => {
        ...
        const cleanedCountry = cleanCountryName(rawCountry);
        const normalizedKey = normalizeCountryForDedup(cleanedCountry);
        ...
        const stats = countryMap.get(normalizedKey);
  ...
  const renderStudentCard = (student) => {
    ...
    // Clean and de-duplicate countries for per-student progress list
    const cleanCountryName = (country) => {
      if (!country) return '';
      return country
        .replace(/[\[\]"]/g, '') // remove brackets and quotes
    ...
    const displayCountries = (student.countries || []).reduce((acc, countryData) => {
      const rawCountry = countryData.country;
      const cleanedCountry = cleanCountryName(rawCountry);
      const normalizedKey = normalizeCountryForDedup(cleanedCountry);

      if (!normalizedKey || seenCountries.has(normalizedKey)) {
        return acc;
      }

      seenCountries.add(normalizedKey);
      acc.push({
        ...countryData,
        country: cleanedCountry
      });
    ...
    return (
      <Card key={student.studentId || student.id} sx={{ mb: 2 }}>
        <CardContent>
            ...
            <Grid item xs={12} md={7}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {displayCountries && displayCountries.length > 0 ? (
                  displayCountries.map((countryData, index) => {
