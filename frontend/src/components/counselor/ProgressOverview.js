...
const ProgressOverview = ({ students = [], countryProgress = [] }) => {
  ...
  // Calculate country-specific completion rates
  const calculateCountryCompletionRates = (countryName) => {
    if (!countryName || countryName === 'all' || !countryProgress || countryProgress.length === 0) {
      return stats.completionRates;
    }

    const countryData = countryProgress.find(cp => cp.country === countryName);
    if (!countryData) {
      return stats.completionRates;
    }

    // Get phase counts for this country
    const countryPhaseCounts = {};
    countryData.phaseDistribution.forEach(phaseData => {
      countryPhaseCounts[phaseData.phase] = phaseData.count;
    });

    // Calculate completion rates for this country
    const countryCompletionRates = PHASES.map((phase, index) => {
    ...
    // Add any unknown phases from country data
    countryData.phaseDistribution.forEach(phaseData => {
      const phaseInfo = PHASES.find(p => p.key === phaseData.phase);
      if (!phaseInfo) {
        // Unknown phase
        const phaseIndex = PHASES.length; // Assume it's after all known phases
        const studentsCompleted = countryData.phaseDistribution
          .filter(pd => {
            const pdIndex = PHASES.findIndex(p => p.key === pd.phase);
            return pdIndex > phaseIndex;
          })
          .reduce((sum, pd) => sum + pd.count, 0);

        countryCompletionRates.push({
          key: phaseData.phase,
          label: getPhaseLabel(phaseData.phase),
          color: getPhaseColor(phaseData.phase),
          completed: studentsCompleted,
          inPhase: phaseData.count,
          completionRate: phaseData.count > 0 ? (studentsCompleted / phaseData.count) * 100 : 0
        });
      }
    });

    return countryCompletionRates;
  };

  const completionRates = calculateCountryCompletionRates(selectedCountry);
    ...
    <Fade in={true} timeout={1000}>
      <Card>
        <CardContent>
          ...
          {/* Country-wise Phase Distribution */}
          {countryProgress && countryProgress.length > 0 && (
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                Phase Distribution by Country
              </Typography>
              {countryProgress.map((countryData) => (
                <Box key={countryData.country} sx={{ mb: 3, p: 2, borderRadius: 2, border: `1px solid ${theme.palette.divider}`, backgroundColor: 'background.paper' }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Chip
                      label={`${countryData.totalStudents} ${countryData.totalStudents === 1 ? 'Student' : 'Students'}`}
                      size="small"
                      color="primary"
                    />
                  </Box>
                  <Grid container spacing={2}>
                    {countryData.phaseDistribution.map((phaseData) => {
                      const phaseInfo = PHASES.find(p => p.key === phaseData.phase) || {
                        label: getPhaseLabel(phaseData.phase),
                        color: getPhaseColor(phaseData.phase)
                      };
                      return (
                        <Grid item xs={12} sm={6} md={4} key={phaseData.phase}>
                          <Box sx={{
                            p: 1.5,
                            borderRadius: 2,
                            border: `1px solid ${theme.palette.divider}`,
                            backgroundColor: phaseData.count > 0 ? `${phaseInfo.color}10` : 'transparent'
                          }}>
                            ...
                            <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block', fontSize: '0.65rem' }}>
                              {phaseData.percentage}% of {countryData.country} students
                            </Typography>
                          </Box>
                        </Grid>
                      );
          ...
          {/* Global Phase Distribution (fallback if no country data) */}
          {(!countryProgress || countryProgress.length === 0) && (
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                Phase Distribution Based On Countries
              </Typography>
            <Grid container spacing={2}>
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 600 }}>
                Phase Completion Rates Based on countries
              </Typography>
              {countryProgress && countryProgress.length > 0 && (
                  ...
                  >
                    ...
                    {countryProgress.map((countryData) => (
                      <MenuItem key={countryData.country} value={countryData.country}>
                        {countryData.country} ({countryData.totalStudents} students)
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
