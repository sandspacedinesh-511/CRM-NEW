...
function MarketingCommunication() {
  ...
  const handleDownloadDocument = async (documentType) => {
    if (!selectedLead || !leadDocuments.length) return;

    // Find the document by type
    const doc = leadDocuments.find(d => d.type === documentType);
    ...
    try {
      ...
      // If URL is a full HTTP/HTTPS URL (likely presigned or public URL)
      if (doc.url.startsWith('http://') || doc.url.startsWith('https://')) {
        // Try to fetch as blob first (works for authenticated URLs)
        try {
          ...
          if (response.ok) {
            ...
            link.href = url;
            link.setAttribute('download', doc.name || `${documentType.toLowerCase().replace('_', '-')}.pdf`);
        ...
        // Fallback: Open URL directly (for presigned URLs or public URLs)
        const link = document.createElement('a');
        link.href = doc.url;
        link.download = doc.name || `${documentType.toLowerCase().replace('_', '-')}.pdf`;
        link.target = '_blank';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        // Relative URL - try using axios with the base URL
        try {
          const response = await axiosInstance.get(doc.url, {
            responseType: 'blob'
          });
                      ...
                      <Box>
                        ...
                        <Stack direction="row" spacing={1}>
                          <Button
                            ...
                            size="small"
                            onClick={() => handleDownloadDocument('ID_CARD')}
                            sx={{ flex: 1 }}
                          >
                            Download
                          </Button>
                      ...
                      <Box>
                        ...
                        <Stack direction="row" spacing={1}>
                          <Button
                            ...
                            startIcon={<DownloadIcon />}
                            size="small"
                            onClick={() => handleDownloadDocument('ENROLLMENT_LETTER')}
                            sx={{ flex: 1 }}
                          >
                            Download
                          </Button>
                      ...
                      <Box>
                        ...
                        <Stack direction="row" spacing={1}>
                          <Button
                            ...
                            startIcon={<DownloadIcon />}
                            size="small"
                            onClick={() => handleDownloadDocument('OTHER')}
                            sx={{ flex: 1 }}
                          >
                            Download
                          </Button>
