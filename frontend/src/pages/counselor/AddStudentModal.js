...
const AddStudentModal = ({ open, onClose, onStudentAdded }) => {
  ...
  const validateEntireForm = () => {
    ...
    if (!formik.values.intendedCountry || formik.values.intendedCountry.length === 0) {
      errors.intendedCountry = 'Select at least one country';
    }
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <FormControl
            ...
            required
          >
            <InputLabel id="intended-country-label">Intended Country *</InputLabel>
            <Select
              labelId="intended-country-label"
              multiple
            ...
            {formik.touched.intendedCountry && formik.errors.intendedCountry && (
              <FormHelperText>{formik.errors.intendedCountry}</FormHelperText>
            )}
            {!formik.errors.intendedCountry && (
              <FormHelperText>Select the countries where the student wants to study</FormHelperText>
            )}
          </FormControl>
          {formik.values.intendedCountry.length > 0 && (
            <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
              {formik.values.intendedCountry.map((country) => (
                <Chip
                  key={country}
                  label={country}
                  size="small"
  ...
  const renderReview = () => {
    ...
    const requiredFields = [
      ...
      { key: 'intendedCountry', label: 'Intended Countries' },
