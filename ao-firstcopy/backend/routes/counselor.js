...
const isCounselor = (req, res, next) => {
  if (req.user.role !== 'counselor') {
    return res.status(403).json({ message: 'Access denied. Counselor only.' });
  }
  next();
};
...
router.post('/students/:id/documents/upload', (req, res, next) => {
  next();
}, upload.single('file'), (req, res, next) => {
  next();
}, documentController.uploadDocument, (error, req, res, next) => {
